...
team_t team = {
    /* Team name */
    "implicit_free_list_dynamic_allocator",
...
static void *extend_heap(size_t words) {
  ...
  return coalesce(bp); // update this statement while coding colaesing
}

/* First fit search */
static void *find_fit(size_t asize) {
  void *bp;
  /* Looping from starting of the heap blocks */
...
/* checking for heap consistency */
int mm_check(void) {

  if ((unsigned int)(heap_listp) == ALIGN(heap_listp)) {
    printf("heap_listp address is %p\n",heap_listp); 
    return 1;
  } 
  return 0;
}
